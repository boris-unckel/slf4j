...
/**
 * Work from https://github.com/apache/logging-log4j2/blob/release-2.x/log4j-api/src/main/java/org/apache/logging/log4j/util/ServiceLoaderUtil.java
 */
...
//          }
     ...
     static <T> Iterable<T> callServiceLoader(Lookup lookup, Class<T> serviceType, ClassLoader classLoader,
          ...
          try {
               ...
               if (verbose) {
                    Util.report("Unable to load services for service " +serviceType,e);
               }
          }
          ...
          public boolean tryAdvance(Consumer<? super S> action) {
               while (serviceIterator.hasNext()) {
                    try {
                         ...
                         Util.report("Unable to load service class for service " + serviceName, e);
                    }
               }
               return false;
          }
